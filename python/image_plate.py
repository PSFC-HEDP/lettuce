import numpy as np
from numpy import interp, exp, log, geomspace, nonzero
...
def xray_sensitivity(
	...
	"""
	return exp(log_xray_sensitivity(energy, filter_stack, ip_type, work_function, psl_attenuation))
...
def log_xray_sensitivity(
	...
	log_sensitivity = log(
		1/work_function*self_transparency*(1 - exp(-attenuation*phosphor_thickness/self_transparency)))
...
def load_attenuation_curve(energy: NDArray[float], material: str) -> NDArray[float]:
	...
	"""
	table = np.loadtxt(f"resources/xray_tables/attenuation_{material}.csv", delimiter=",")
	return interp(energy, table[:, 0], table[:, 1])
...
def fade(time: float, A1=.436, A2=.403, τ1=1.134e3, τ2=9.85e4):
	...
	"""
	return A1*exp(-time/τ1) + A2*exp(-time/τ2) + (1 - A1 - A2)
